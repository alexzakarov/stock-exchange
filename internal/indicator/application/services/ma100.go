package services

import (
	"context"
	ent "main/internal/indicator/domain/entities"
	"main/pkg/market_data/binance"
	fx "main/pkg/utils/formulas"
)

// IndicatorMa100 Indicator Calculate Func
func (u *service) IndicatorMa100(ctx context.Context, chartData []binance.ChartData) (responser ent.IndicatorCalcResponse, err error) {
	if u.cfg.Server.APP_DEBUG == true {
		println("IndicatorMa100 begin to work")
	}

	var closeLine []float64
	var signal int
	var result []float64
	period := 100

	for _, values := range chartData {
		closeLine = append(closeLine, values.ClosePrice)
	}

	ma := fx.MA(period, closeLine)
	for i := 0; i < len(chartData); i++ {
		if closeLine[i] > ma[i] {
			result = append(result, 1)
		} else if closeLine[i] < ma[i] {
			result = append(result, -1)
		} else {
			result = append(result, 0)
		}
	}

	signal = int(fx.GetByIndexN(result, 2))

	responser = ent.IndicatorCalcResponse{
		Signal:       signal,
		Result:       nil,
		CalculatedAt: chartData[len(chartData)-1].CloseTime,
	}

	return
}
