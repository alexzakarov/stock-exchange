...
func (r *postgresqlRepo) Register(ctx context.Context, req ent.RegisterReq) (record int64) {
	...
	if req.UserType == 1 {
		query = `INSERT INTO auth (src, company_name, user_title, user_name, user_pass) VALUES ($1,$2,$3,$4,$5) RETURNING auth_id`
		errDb = r.db.QueryRow(ctx, query, req.Src, req.CompanyName, req.UserTitle, req.UserName, req.UserPass).Scan(&auth_id)
	} else {
		query = `INSERT INTO auth (src, user_title, user_name, user_pass) VALUES ($1,$2,$3,$4) RETURNING auth_id`
...
func (r *postgresqlRepo) Login(ctx context.Context, req ent.LoginReq) (record int64, auth ent.Auth) {
	...
	if req.UserType == 1 {
		query = `SELECT auth_id, lang, parent_id, user_type, acc_type, COALESCE(company_name,''), user_title, user_name, is_demo, unique_id, status FROM auth WHERE user_name=$1 AND user_pass=$2`
		...
		query = `SELECT auth_id, lang, manager_id, user_type, user_title, user_name, unique_id, status FROM auth WHERE user_name=$1 AND user_pass=$2`
...
func (r *postgresqlRepo) SaveContact(ctx context.Context, auth_id int64, req ent.RegisterReq) (record int64) {
	...
	if req.UserType == 1 {
		query = `INSERT INTO auth_contact(auth_id, type_of, description, definition, is_default) VALUES ($1,$2,$3,$4,$5)`
